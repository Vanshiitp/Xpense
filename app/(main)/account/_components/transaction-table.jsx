...
export function TransactionTable({ transactions }) {
  ...
  const filteredAndSortedTransactions = useMemo(() => {
    ...
    // Apply sorting
    // a-b == -ve indicated a comes before b and vice versa
    result.sort((a, b) => {
      let comparison = 0;
  ...
  const filteredCategories = useMemo(() => {
    if (!typeFilter) return defaultCategories;
    return defaultCategories.filter(category => category.type === typeFilter);
  }, [typeFilter]);
  ...
  const handleSelect = (id) => {
    setSelectedIds((current) =>
      current.includes(id)
        ? current.filter((item) => item !== id) //deselecting the id if already selected
        : [...current, id] //selecting the id if not selected
    );
  };
